...
const STEPS: readonly Step[] = [
  ...
  { kind: "explain", selector: ".all-device-grid", title: "受付端末一覧", body: "ここに現在稼働している入口・出口iPadが並びます。まず何台見えているかと、それぞれの状態を確認します。", note: "入口か出口のどちらかが消えている場合は、通信断や受付アプリ停止の可能性があります。" },
  { kind: "explain", text: "最終通信", closest: ".device-card", title: "最終通信を見る", body: "『最終通信』は、そのiPadから最後に状態が届いた時刻です。数秒前なら正常です。時間が大きく空いている場合は、その端末のWi-Fiや受付画面を確認します。" },
  { kind: "explain", text: "Firebase応答", closest: ".device-card", title: "Firebase応答を見る", body: "これは受付端末とサーバーの応答速度です。数字が大きくなり続ける場合、通信が不安定な可能性があります。" },
...
export default function StandaloneTutorialGuideV3() {
  ...
  const progress = useMemo(() => completed ? 100 : Math.round(((stepIndex + 1) / STEPS.length) * 100), [completed, stepIndex]);
  ...
  const start = () => { setStepIndex(0); setTargetReady(false); setActive(true); };
  const close = () => { setActive(false); setStepIndex(0); setTargetReady(false); };
  const next = () => { if (step?.kind === "explain") { setTargetReady(false); setStepIndex((current) => current + 1); } };
  ...
  return <>
    ...
    {active && <section className="tutorial-v3-card" aria-live="polite">
      {completed ? <>
        ...
        <h2>実物を見ながら基本操作を確認できました</h2><p>各画面の人数、予測、端末状態、障害、自動運転、AI管制の実物を順番に確認しました。</p>
        <div className="tutorial-v3-progress"><span style={{width:"100%"}}/></div><div className="tutorial-v3-actions"><button type="button" onClick={start}>もう一度</button><button type="button" className="primary" onClick={close}>終了</button></div>
      </> : step !== undefined ? <>
        <div className="tutorial-v3-meta"><span>{step.kind === "menu" ? "MISSION" : "SCREEN GUIDE"}</span><span>{stepIndex + 1} / {STEPS.length}</span></div>
        <h2>{step.title}</h2><p>{step.body}</p>{step.note !== undefined && <p className="tutorial-v3-note">ポイント：{step.note}</p>}
        <p className="tutorial-v3-instruction">{step.kind === "menu" ? targetReady ? "光っているメニューをタップしてください。" : "対象メニューを探しています…" : targetReady ? "光っている実物を見ながら説明を確認してください。読み終わったら『次へ』を押します。" : "遷移後の画面を待っています…"}</p>
        <div className="tutorial-v3-progress"><span style={{width:`${progress}%`}}/></div><div className="tutorial-v3-actions"><button type="button" onClick={close}>終了</button>{step.kind === "explain" && <button type="button" className="primary" onClick={next} disabled={!targetReady}>次へ</button>}</div>
      </> : null}
    </section>}
  </>;
}
