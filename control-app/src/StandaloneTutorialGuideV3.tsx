...
const BASIC_STEPS: readonly Step[] = [
  ...
  { kind: "explain", selector: ".all-device-grid", title: "受付端末一覧", body: "ここに現在稼働している入口・出口iPadが並びます。まず何台見えているかと、それぞれの状態を確認します。", note: "端末が0台でもチュートリアルは続けられます。入口か出口が消えている場合は、通信断や受付アプリ停止の可能性があります。" },
  { kind: "explain", text: "最終通信", closest: ".device-card", title: "最終通信を見る", body: "『最終通信』は、そのiPadから最後に状態が届いた時刻です。数秒前なら正常です。時間が大きく空いている場合は、その端末のWi-Fiや受付画面を確認します。", note: "現在オンラインの端末がない場合、この項目は表示されないためスキップできます。" },
  { kind: "explain", text: "Firebase応答", closest: ".device-card", title: "Firebase応答を見る", body: "これは受付端末とサーバーの応答速度です。数字が大きくなり続ける場合、通信が不安定な可能性があります。", note: "現在オンラインの端末がない場合、この項目は表示されないためスキップできます。" },
...
const LAB_STEPS: readonly Step[] = [
  { kind: "menu", menuLabel: "管制ラボ", title: "管制ラボを開く", body: "左の『管制ラボ』を押してください。ここは通常運用とは分けて用意している試験機能です。自動運転の状態と判断範囲を確認します。" },
  { kind: "explain", selector: ".lab-status-hero", title: "現在の自動運転レベル", body: "ここが現在の自動運転レベルです。OFF、Lv.1支援、Lv.2半自動、Lv.3自動のどこで動いているかと、現在の判断対象件数を確認できます。", note: "試験機能なので、本番で使う場合も状態を確認しながら運用します。" },
  { kind: "explain", selector: ".autopilot-level-grid", title: "自動運転レベルの違い", body: "Lv.1は提案だけ、Lv.2は人の承認後に操作、Lv.3は安全な復旧操作を自動実行します。ここを押すと実際の設定が変わるので、ガイド中は見るだけでOKです。", note: "受付停止・受付再開のように運用へ直接影響する操作は、人の確認が必要です。" },
];

const COPILOT_STEPS: readonly Step[] = [
  { kind: "menu", menuLabel: "AI管制", title: "AI管制を開く", body: "左の『AI管制』を押してください。ライブデータを文章で確認する試験機能のコパイロット画面を見ます。" },
  ...
  { kind: "explain", selector: ".copilot-page", title: "管制コパイロット", body: "『今どんな状態？』『入口の通信を調べて』『一番確認すべきことは？』のように、そのまま文章で質問できます。遠隔操作につながる内容は、対象と操作内容を確認してから実行する仕組みです。", note: "現在は外部AIではなく、ライブデータとルール推論を使う疑似AIです。試験機能として通常操作とは分けて扱います。" },
];

const GUIDE_INFO: Record<TutorialMode, { label: string; completeTitle: string; completeBody: string }> = {
  basic: {
    label: "基本操作ガイド",
    completeTitle: "基本操作を確認できました",
    completeBody: "ライブ運行、端末、障害履歴の基本的な見方を実物で確認しました。",
  },
  lab: {
    label: "試験機能 · 管制ラボ",
    completeTitle: "管制ラボの説明は完了です",
    completeBody: "自動運転レベルと、どこまで自動操作するかの考え方を確認しました。",
  },
  copilot: {
    label: "試験機能 · AI管制",
    completeTitle: "AI管制の説明は完了です",
    completeBody: "疑似AIが見るライブデータと、質問・遠隔操作の扱いを確認しました。",
  },
};

function stepsFor(mode: TutorialMode) {
  if (mode === "lab") return LAB_STEPS;
  if (mode === "copilot") return COPILOT_STEPS;
  return BASIC_STEPS;
}
...
function readMode(detail: unknown): TutorialMode | null {
  return detail === "basic" || detail === "lab" || detail === "copilot" ? detail : null;
}
...
export default function StandaloneTutorialGuideV3() {
  ...
  const guideInfo = GUIDE_INFO[mode];
  ...
  useEffect(() => {
    const startFromHome = (event: Event) => {
      if (!(event instanceof CustomEvent)) return;
      const nextMode = readMode(event.detail);
    ...
    window.addEventListener("control-tutorial:start", startFromHome);
    return () => window.removeEventListener("control-tutorial:start", startFromHome);
  }, []);
  ...
  return <>
    <style>{`
      .tutorial-v3-card{position:fixed!important;right:max(16px,env(safe-area-inset-right))!important;bottom:max(16px,env(safe-area-inset-bottom))!important;z-index:2147483646!important;width:min(470px,calc(100vw - 32px))!important;max-height:min(68dvh,620px)!important;overflow:auto!important;box-sizing:border-box!important;padding:18px!important;border:1px solid rgba(255,255,255,.94)!important;border-radius:22px!important;background:rgba(255,255,255,.98)!important;color:#172033!important;box-shadow:0 24px 70px rgba(20,17,45,.34)!important}
      .tutorial-v3-meta{display:flex!important;justify-content:space-between!important;gap:12px!important;color:#7352d6!important;font-size:12px!important;font-weight:900!important;letter-spacing:.08em!important}.tutorial-v3-mode{margin-top:6px!important;color:#6a7485!important;font-size:11px!important;font-weight:850!important}
      ...
      @media(max-width:760px){.tutorial-v3-card{width:calc(100vw - 24px)!important;right:12px!important}}
    `}</style>
    ...
    {active && <section className="tutorial-v3-card" aria-live="polite">
      {completed ? <>
        ...
        <div className="tutorial-v3-mode">{guideInfo.label}</div>
        <h2>{guideInfo.completeTitle}</h2>
        <p>{guideInfo.completeBody}</p>
        <div className="tutorial-v3-progress"><span style={{ width: "100%" }} /></div>
        <div className="tutorial-v3-actions"><button type="button" onClick={restart}>もう一度</button><button type="button" className="primary" onClick={close}>終了</button></div>
      </> : step !== undefined ? <>
